...
- **固定 9 个槽位**：椭圆牌桌周围预设 `seat-pos-0` … `seat-pos-8`；人数少时只占用其中一部分，不会挤满整圈
- **「我」始终在底部正中**（`seat-pos-0`）：渲染前会把当前玩家旋转到列表末尾，再按人数映射到对应槽位
- **7 人及以上**：牌桌座位区启用 `is-dense`，略微收紧左右与底弧位置，减少重叠
- **满 9 人**：额外启用 `is-full-table`，缩小座位卡片与头像；第 9 个额外槽位 `seat-pos-8` 在底弧右侧（与底部正中对称），避免与「我」重叠
- **局内中途加入**：不在牌桌圆环上显示，出现在「等待入局」列表；下一局 `startHand` 时入座
